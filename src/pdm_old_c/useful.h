...
#define COMPILER_ERROR_VA(M, ...) __compilerErrorHandler(__FILE__,__LINE__,NULL,M,__VA_ARGS__)
#define COMPILER_ASSERT_VA(C,M,...) ((C) ? (1) : (__compilerErrorHandler(__FILE__,__LINE__,"Assertion failed: (" #C ")", M, __VA_ARGS__)))

#define COMPILER_ERROR(M) (__compilerErrorHandler(__FILE__,__LINE__,NULL,M))
