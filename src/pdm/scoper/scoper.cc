    ...
    bool ScoperVisitor::on_visit__mod_stmt(ast::ModStmt* node, VisitOrder visit_order) {
        if (visit_order == VisitOrder::Pre) {
            ...
            types::TypeVar* module_tv; {
                ...
                module_tv = scoper()->types_mgr()->new_unknown_tv(std::move(tv_name), node);
            }
    ...
    bool ScoperVisitor::on_visit__mod_type_stmt(ast::ModTypeStmt* node, VisitOrder visit_order) {
        if (visit_order == VisitOrder::Pre) {
            ...
            if (node->lhs_tpatterns().empty()) {
                ...
                type_var = scoper()->types_mgr()->new_unknown_tv(std::move(tv_name));
            } else {
    ...
    bool ScoperVisitor::on_visit__mod_enum_stmt(ast::ModEnumStmt* node, VisitOrder visit_order) {
        if (visit_order == VisitOrder::Pre) {
            ...
            if (node->tpatterns().empty()) {
                ...
                enum_var = scoper()->types_mgr()->new_unknown_tv(std::move(tv_name), node);
            } else {
                // template function
    ...
    bool ScoperVisitor::on_visit__extern_stmt(ast::ExternStmt* node, VisitOrder visit_order) {
        if (visit_order == VisitOrder::Pre) {
            types::TypeVar* ext_mod_tv = nullptr; {
                ...
                ext_mod_tv = scoper()->types_mgr()->new_unknown_tv(std::move(tv_name), node);
            }
    ...
    bool ScoperVisitor::on_visit__import_stmt(ast::ImportStmt* node, VisitOrder visit_order) {
        if (visit_order == VisitOrder::Pre) {
            ...
            types::TypeVar* mod_tv = nullptr; {
                ...
                mod_tv = scoper()->types_mgr()->new_unknown_tv(std::move(tv_name), node);
            }
            Defn new_defn {
    ...
    bool ScoperVisitor::on_visit__vpattern(ast::VPattern* node, VisitOrder visit_order) {
        if (visit_order == VisitOrder::Pre) {
            for (ast::VPattern::Field* field: node->fields()) {
                ...
                types::TypeVar* field_tv = scoper()->types_mgr()->new_unknown_tv(std::move(tv_name), node);
                Defn new_defn {
        ...
        if (visit_order == VisitOrder::Pre) {
            for (ast::TPattern::Field* field: node->fields()) {
                ...
                if (field->kind() == ast::TPattern::FieldKind::Value) {
                    // storing typeof arg:
                    field_var = scoper()->types_mgr()->new_unknown_tv(std::move(tv_name), node);
